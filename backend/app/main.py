...
async def upload_image(
    ...
    db_image = crud.create_image_metadata(
        db=db, image_path=file_location, label=label, image_metadata=image_metadata
    )
...
async def update_image(
    image_id: int, request: schemas.ImageMetadataUpdate, db: Session = Depends(get_db)
):
    updated_image = crud.update_image_metadata(
        db=db,
        image_id=image_id,
        label=request.label,
